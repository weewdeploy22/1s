...
var global_currentNodeIndex = 999;
// var allTasks = [];
// var todoTasks = [];
var todoTasks = [
...
const refreshTasks_fromTodo_1s = function () {
    const localTempTasks = todoTasks.filter(e => e.titleLevel === '1s')[global_currentNodeIndex].tasks;
    if (localTempTasks.length > 0) {
        sectionTasks_time = Date.now() + '';
        for (let i = 0; i < localTempTasks.length; i++) {
            const task = localTempTasks[i];
...
export async function main(
    ...
    global_currentNodeIndex = currentNodeIndex;
